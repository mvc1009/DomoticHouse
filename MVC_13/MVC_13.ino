...
//====VARIABLES SERVO=======
Servo puerta;
const byte led_servo_R = 36;
const byte led_servo_G = 37;
const byte led_servo_B = 38;
...
boolean SENSOR_SW420 = false;
boolean SENSOR_SW420_ANTES = false;
boolean Config_SW420 = false;

String SW420_ON = "ALARMA VENTANA ON";
String SW420_OFF = "ALARMA VENTANA OFF";
//============VARIABLES IRFLAME=========================
...
motor.setSpeed(10);
  ...
  pinMode (bomb220, OUTPUT);
  pinMode (VENT_HABITACION, OUTPUT);
  pinMode (VENT_COMEDOR, OUTPUT);
  
  pinMode (SW420, INPUT ); //SENSOR VIBRACION VENTANA
   
  pinMode (HCSR501_COMEDOR, INPUT ); //SENSOR MOVIMIENTO COMEDOR
  pinMode (HCSR501_JARDIN, INPUT); //SENSOR MOVIMIENTO JARDIN
 
  pinMode (led_servo_R, OUTPUT ); //LED SERVO
  pinMode (led_servo_G, OUTPUT );
  pinMode (led_servo_B, OUTPUT );
  ...
  pinMode (LM35_COMEDOR, INPUT);   //SENSOR TEMP COMEDOR 
  pinMode (LED_AA_COMEDOR, OUTPUT ); //LEDs AA o CC   COMEDOR
  pinMode (LED_CC_COMEDOR, OUTPUT );
  pinMode (LM35_HABITACION, INPUT); //SENSOR TEMP HABITACION
  pinMode (LED_AA_HABITACION, OUTPUT ); //LEDs AA o CC   HABITACION
  pinMode (LED_CC_HABITACION, OUTPUT );

  pinMode (altavoz, OUTPUT);
  pinMode (led_alarma, OUTPUT );
  pinMode (led_no_alarma, OUTPUT);
  ...
  puerta.write (90); //puerta en reposo (cerrada)
  delay(2000);

  digitalWrite (VENT_COMEDOR, HIGH);   //VENTILADOR HIGH ES APAGAR EL VENTILADOR ESTA INVERTIDO, y LOW ES ENCENDIDO
  digitalWrite (VENT_HABITACION, HIGH);
  digitalWrite (led_no_alarma, HIGH);
...
// ===============================================
 ...
 comando_bt = comando_bt + comando_bt_recivido;
    ...
    String porcentaje_altura_persiana = (comando_bt.substring(1, 3)); /*el comando que recibira sera
   #XX% y a nosotros solo nos interesa el XX que sera el porcentaje y solo leemos esos dos numeros la posicion 1 y 3 */
    ...
    ALTURA_INICIAL = ALTURA_DESTINO; //cambiamos el valor de la altura inicio a la altura destino
      ...
      if (comando_bt == "142") {
      Config_SW420 = true;
        Serial.println ("SENSOR DE VENTANA ACTIVADO");    
     Linea4 = "  SENSR VENTANA ON  ";
     scroll_lcd ();
    }
      if (comando_bt == "042") {
      Config_SW420 = false;
      Serial.println ("SENSOR DE VENTANA DESACTIVADO");    
         Linea4 = " SENSR VENTANA OFF  "; 
        scroll_lcd();
    }
     if (comando_bt == "145") {
    ...
    if (comando_bt == "0A2") {
      ...
      digitalWrite(VENT_HABITACION, LOW);
    ...
    if (comando_bt == "139") {
      digitalWrite (bomb220, HIGH);    
      }
    if (comando_bt == "039") {
      digitalWrite (bomb220, LOW);    
      }
    if (comando_bt == "271"){
    ...
    if (comando_bt = "320") {
      digitalWrite (VENT_COMEDOR, HIGH);
      digitalWrite (LED_AA_COMEDOR, LOW);
      digitalWrite (LED_CC_COMEDOR, LOW);
    }
    if (comando_bt == "321"){
    ...
    if (comando_bt == "109") {
      ...
      puerta.write (0); //abrir puerta 90º
      
      digitalWrite (led_servo_R, HIGH);
      digitalWrite (led_servo_G, HIGH);
      digitalWrite (led_servo_B, HIGH);
      delay (1000);
      digitalWrite (led_servo_R, LOW);
      digitalWrite (led_servo_G, LOW);
      digitalWrite (led_servo_B, LOW);
      delay (1000);
      digitalWrite (led_servo_R, HIGH);
      digitalWrite (led_servo_G, HIGH);
      digitalWrite (led_servo_B, HIGH);
      ...
      Serial.println ("PUERTA FINCA ABIERTA");
      
      Linea4 = (HORA_ACTUAL);
      scroll_lcd();
      Linea4 = "PUERTA FINCA ABIERTA";
      scroll_lcd();
    }
    if (comando_bt == "009") {
      ...
      puerta.write (90); //cerrar puerta 0º
      digitalWrite (led_servo_R, LOW);
      digitalWrite (led_servo_G, LOW);
      digitalWrite (led_servo_B, LOW);
      delay (1000);
      ...
      Serial.print (HORA_ACTUAL);
      Serial.println ("PUERTA FINCA CERRADA");
      
      Linea4 = (HORA_ACTUAL);
      scroll_lcd();
      Linea4 = "PUERTA FINCA CERRADA";
      scroll_lcd();
    }
    ...
    SENSOR_IRFLAME_COMEDOR = digitalRead (IRFLAME_COMEDOR);  //LEEMOS EL SENSOR SI HAY FUEGO O NO
         ...
         HORA_ACTUAL = HORA_ACTUAL + "   ";
           ...
           Linea4 = "ALARMA FUEGO COM OFF";
             ...
             digitalWrite (led_no_alarma, LOW);
...
if (Config_LM35_HABITACION) {
      ...
      TEMP_CONFIG_LM35_HABITACION_ANTES = TEMP_CONFIG_LM35_HABITACION;
        ...
        digitalWrite (VENT_HABITACION, HIGH);// =====  final del (Config_LM35_HABITACION)
      }
...
//====================SW420=========================
    ...
    SENSOR_SW420 = digitalRead (SW420);  //LEEMOS EL SENSOR SI HAY VIBRACIÓN EN LA VENTANA
    
         if (SENSOR_SW420 != SENSOR_SW420_ANTES){    // comparamos si la variable actual es igual que la anterior
         HORA_ACTUAL = readDS3231time(); // HORA_ACTUAL ="DD/MM/AA HH:MM:SS" texto de 17 caracteres
         HORA_ACTUAL = HORA_ACTUAL + "   ";
           ...
           if  (!SENSOR_SW420) {
            ...
            Serial.print ( HORA_ACTUAL );
            Serial.println ("VENTANA FORZADA");

            comando_enviado_bt = HORA_ACTUAL + SW420_ON;  //enviar al bluetooth un emergente
           Serial1.println (comando_enviado_bt);
           
            Linea4 = (HORA_ACTUAL);   //imprimir en el LCD
            scroll_lcd ();
            Linea4 = "  VENTANA FORZADA   ";
            }
             scroll_lcd ();
            SENSOR_SW420_ANTES = SENSOR_SW420;  // cambiamos el valor de la variable del sensor para que la lectura actual al siguiente loop sea la anterior
          }
  }
//==============SENSOR KY003 JARDIN===================================
